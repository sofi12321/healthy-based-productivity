...
class Planner:
    ...
    def set_available_time_slots(self, user_start_time, user_end_time, plan_time, tasks, events):
        ...
        # if plan_time >= user_start_time:
        #     # check
        #     user_start_time = user_start_time + plan_time
        #     user_start_time = (user_start_time - plan_time).total_seconds() / 60 / 60 / 24
        #     user_end_time = (user_end_time - plan_time).total_seconds() / 60 / 60 / 24
        #
        # else:
        #     user_start_time = (user_start_time - plan_time).total_seconds() / 60 / 60 / 24
        #     user_end_time = (user_end_time - plan_time).total_seconds() / 60 / 60 / 24
        #
        #     self.update_available_time_slots_task((0, user_end_time, 0))
        #     self.update_available_time_slots_task((user_start_time, 1, 0))
    ...
    def update_available_time_slots_task(self, prediction):
        ...
        self.available_time_slots = self.update_slot(
            self.round_precision(time), self.round_precision(max(duration, duration + offset)),
    ...
    def convert_output_to_schedule(self, task, prediction, plan_time):
        """
        ...
        :param task: id of the task in a database
        ...
        time, duration, offset = prediction[0], prediction[1], prediction[2]
        time, duration, offset = self.check_model_output(task, time, duration, offset)
        print("Our schedule ", time, duration, offset)
    ...
    def find_time_slot(self, start_time, end_time):
        start_slot = 0
        end_slot = 0
        for slot in range(len(self.available_time_slots) - 1):
            if self.available_time_slots[slot][0] <= start_time <= self.available_time_slots[slot + 1][0]:
                start_slot = slot
            if self.available_time_slots[slot][0] <= end_time <= self.available_time_slots[slot + 1][0]:
                end_slot = slot
                return start_slot, end_slot
        return start_slot, end_slot

    def check_model_output(self, task, predicted_time, predicted_duration, predicted_offset,
                           delta_start=1 * 60 / 1440):
        if not task.start_time:
            user_start_time, user_duration, _ = self.convert_history_to_output(datetime.datetime.now().date(),
                                                                               datetime.datetime.now().time(),
                                                                               task.duration, task.duration)
            user_start_time = predicted_time
            if not 8 * 60 / 1440 < user_start_time < 21 * 60 / 1440:
                user_start_time = 14 * 60 / 1440

        else:
            user_start_time, user_duration, _ = self.converter.user_to_model(
                ...
                offset=0,
                current_date=self.plan_time
            )

        # First layer checks
        if predicted_offset < 0:
            predicted_offset = -predicted_offset
        if user_duration >= predicted_duration:
            predicted_offset = max(0, predicted_duration + predicted_offset - user_duration)
            predicted_duration = user_duration
        else:
            predicted_offset = predicted_offset + predicted_duration - user_duration
            predicted_duration -= user_duration

        if user_duration != 0 and predicted_offset > user_duration / 2:
            predicted_offset = user_duration / 2

        # Check ability to set
        # fall_user_time = False
        if predicted_time < user_start_time - delta_start:
            predicted_time = user_start_time - delta_start * 0.5
        elif predicted_time > user_start_time + delta_start:
            predicted_time = user_start_time + delta_start * 0.5

        if predicted_offset < 10 / 24 / 60:
            predicted_offset = 10 / 24 / 60

        start_slot, end_slot = self.find_time_slot(predicted_time,
                                                   predicted_time + predicted_duration + predicted_offset)
        if end_slot < start_slot:
            end_slot = start_slot
        if start_slot != end_slot and (self.available_time_slots[start_slot][1] < predicted_time or
                                       predicted_time - self.available_time_slots[start_slot][1] <
                                       min(predicted_time + predicted_duration + predicted_offset,
                                           self.available_time_slots[end_slot][1]) -
                                       self.available_time_slots[end_slot][0]):
            start_slot = end_slot
            predicted_time = self.available_time_slots[start_slot][0]

        for i in range(max(start_slot, len(self.available_time_slots) - start_slot)):
            for j in range(2):
                if i != 0 and j == 0:
                    #         +
                    if not self.available_time_slots[start_slot + i][0] <= predicted_time < \
                           self.available_time_slots[start_slot + i][1]:
                        # left end near thean right
                        predicted_time = self.available_time_slots[start_slot + i][
                                             1] - predicted_duration - predicted_offset
                elif i != 0 and j == 1:
                    i = -i
                    if not self.available_time_slots[start_slot + i][0] <= predicted_time < \
                           self.available_time_slots[start_slot + i][1]:
                        predicted_time = self.available_time_slots[start_slot + i][0]

                if self.available_time_slots[start_slot + i][1] - self.available_time_slots[start_slot + i][0] < \
                        predicted_duration:
                    # Does it suits at min in a slot
                    continue
                if self.available_time_slots[start_slot + i][1] - self.available_time_slots[start_slot + i][0] <= \
                        predicted_duration + predicted_offset:
                    predicted_time = self.available_time_slots[start_slot + i][0]
                    predicted_offset = self.available_time_slots[start_slot + i][1] - \
                                       self.available_time_slots[start_slot + i][0] - predicted_duration
                    return predicted_time, predicted_duration, predicted_offset

                if self.available_time_slots[start_slot + i][0] <= predicted_time and \
                        predicted_time + predicted_duration + predicted_offset < \
                        self.available_time_slots[start_slot + i][1]:
                    return predicted_time, predicted_duration, predicted_offset

                delta = predicted_time + predicted_duration + predicted_offset - \
                        self.available_time_slots[start_slot + i][1]
                if self.available_time_slots[start_slot + i][0] < predicted_time - delta:
                    predicted_time -= delta
                    return predicted_time, predicted_duration, predicted_offset
    ...
    def get_model_schedule(
        ...
        for event in events_new:
            ...
            _, user_h, user_c = self.call_model(
                input_vector, activity_type, [[0.0, 1.0]], user_h, user_c
            )
            self.update_available_time_slots_event(event)
